module username_registry::username_registry {
    ...
    public struct UsernameChanged has copy, drop {
        user: address,
        old_username: String,
        new_username: String,
    }
    ...
    public entry fun set_username(
        ...
        // Check if user already has a username and remove it first
        let old_username = if (table::contains(&registry.user_to_username, user)) {
            let old = table::borrow(&registry.user_to_username, user);
            string::bytes(old)
        } else {
            vector::empty<u8>()
        };

        // If user had a previous username, remove it
        if (vector::length(&old_username) > 0) {
            let old_username_str = string::from_bytes(old_username);
            table::remove(&mut registry.username_to_user, &old_username_str);
            vec_set::remove(&mut registry.taken_usernames, &old_username_str);
            string::drop(old_username_str);
        };

        // Now check if username is already taken by someone else
