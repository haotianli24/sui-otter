...
function extractTokenSymbol(typeStr: string): string {
...
function formatObjectDescription(change: any, sender: string, type: string): string {
    ...
    if (typeName.toLowerCase().includes('nft') ||
                ...
                const getMoveCallDescription = (module: string, functionName: string) => {
                    ...
                    if (functionName.includes('swap')) {
                        return protocolName !== 'Unknown Protocol'
                            ? `Called swap on ${protocolName}`
                            : `${senderShort} executed token swap`;
                    } else if (functionName.includes('deposit')) {
                        return protocolName !== 'Unknown Protocol'
                            ? `Deposited to ${protocolName}`
                            : `${senderShort} executed deposit`;
                    } else if (functionName.includes('withdraw')) {
                        return protocolName !== 'Unknown Protocol'
                            ? `Withdrew from ${protocolName}`
                            : `${senderShort} executed withdrawal`;
                    } else if (functionName.includes('transfer')) {
                        return protocolName !== 'Unknown Protocol'
                            ? `Transferred via ${protocolName}`
                            : `${senderShort} executed transfer`;
                    } else if (functionName.includes('mint')) {
                        return protocolName !== 'Unknown Protocol'
                            ? `Minted via ${protocolName}`
                            : `${senderShort} executed mint`;
                    } else if (functionName.includes('burn')) {
                        return protocolName !== 'Unknown Protocol'
                            ? `Burned via ${protocolName}`
                            : `${senderShort} executed burn`;
                    } else if (functionName.includes('input_signal')) {
                        return `Called ${functionNameClean} on ${protocolName}`;
                    } else {
                        return protocolName !== 'Unknown Protocol'
                            ? `Called ${functionNameClean} on ${protocolName}`
                            : `${senderShort} executed ${functionNameClean}`;
                    }
                };
